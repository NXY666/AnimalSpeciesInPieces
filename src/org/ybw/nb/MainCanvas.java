...
public class MainCanvas extends JPanel {
    ...
    class ThreadGroup extends Thread {
        ThreadSingle[] threadSingles = new ThreadSingle[33];

        ThreadGroup() {
            for (int i = 0; i < 33; i++) {
                threadSingles[i] = new ThreadSingle(triangleNode[i], i + 1);
            }
        }
    ...
    class ThreadSingle extends Thread {
        TriangleNode triangleNode;

        int          index;

        public ThreadSingle(TriangleNode triangleNode, int index) {
            this.triangleNode = triangleNode;
            this.index = index;
        }
        ...
        public void run() {
            ...
            try {
                while (true) {
                    for (int i = 0; i < 3; i++) {
                        triangleNode.movePoint(i, triangleNode.nowP, triangleNode.setP);
                    }
                    sleep(3+index/5 );
                }
            } catch (InterruptedException e) {
